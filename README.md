...
```
$ java -jar generator.jar generate --max-rows=10000 --overwrite profile.json out.csv
...
Finally, before exploring some more interesting features of the generator, we'll add a field for National Insurance number. In this case, the constraints applied to the field ensure that it only has a value if the age is greater than 15:
  ...
  "rules": [
    ...
    },
    {
      ...
      "constraints": [
        ...
        { "not": { "field": "nationalInsurance", "is": "null" } },
        {
          "if": {
            "field": "age",
            "is": "greaterThan",
...
The generator supports a number of different generation modes:
 ...
 - **random** - generates random data that abides by the given set of constraints, with the potentially infinite output limited via the `--num-rows` option.
...
The mode is specified via the `--generation-mode` option. The following example outputs 'inetersting' vaues for the current profile:

~~~
$ java -jar generator.jar generate --generation-type interesting --overwrite profile.json out.csv
~~~
...
If you take a look at the generated file `1.csv` you'll see that data for the `firstName` field does not objy the given constraints, whilst those for `age` anf `nationalInsurance` are correct:

~~~
firstName,age,nationalInsurance
-619248029,71,"HT849919"
08-12-2001 02:53:16,15,"N/A"
"Lorem Ipsum",11,"N/A"
...
That's the end of our getting started guide. Hopefully it has given you a good understanding of what the DataHelix generator is capable. If you'd like to find out more about the various constraints the tools supports, the [Profile Developer Guide](???) is a good next step.
